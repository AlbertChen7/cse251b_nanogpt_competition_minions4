"""
GPT Language Model from Karpathy nanoGPT
"""
...
def load_model(checkpoint_path: str, device: str = "cuda") -> torch.nn.Module:
    ...
    model = nanoGPT(config=MODEL_CONFIG)  # Use your config here
...
#     checkpoints = [f for f in os.listdir() if f.endswith('.pt')]
#     if not checkpoints:
#         print("Creating example model...")
#         model_config = nanoGPTConfig(block_size=1024, n_embd=576)
#         model = nanoGPT(config=MODEL_CONFIG)
#         n_params = sum(p.numel() for p in model.parameters())
#         print(f"Parameters: {n_params:,}")
#     else:
#         print("Loading existing model checkpoint...")
#         model = load_model(checkpoint_path=checkpoints[-1], device='cpu')
#         n_params = sum(p.numel() for p in model.parameters())
#         print(f"Parameters: {n_params:,}")

#     # Test forward pass
#     dummy_input = torch.randint(0, 50257, (8, 1024))
#     logits = model(dummy_input)
#     print(f"Input shape:  {dummy_input.shape}")
#     print(f"Output shape: {logits.shape}")

#     # Save example checkpoint
#     checkpoint_path = f"{os.path.dirname(os.path.abspath(__file__))}/checkpoint.pt"
