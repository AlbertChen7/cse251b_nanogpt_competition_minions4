...
class GPTConfig:
    ...
    n_head: int = 10        # 640 / 10 = 64 head dimension
    n_embd: int = 640       # Decreased from 768
    use_rope: bool = True
    rope_base: float = 10000.0
    mlp_type: str = "swiglu"
...
if __name__ == "__main__":
    ...
    # load teacher for distill
    teacher_model = GPT.from_pretrained("gpt2").to(device)
    teacher_model.eval()
    teacher_model.requires_grad_(False)
    if use_compile:
        teacher_model = torch.compile(teacher_model)

    alpha_kd = 0.5 # weight for distill
    temp_kd = 2.0 # temp for smoothing
    ...
    for step in range(start_step, max_steps):
        ...
        # ema update
        with torch.no_grad():
            for param, ema_param in zip(raw_model.parameters(), ema_model.parameters()):
